...
MKLWeibullRNG(_In_ const MKL_INT nvalues,
	...
	m_rvec    = lam::common::lam_edmalloca(static_cast<size_t>(nvalues), align64B); // <-- Upon touching m_rvec load 64byte(whole object) to cache
	m_alpha   = alpha;
	m_a       = a;
	m_beta    = beta;
	m_nvalues = nvalues;
	...
	m_seed    = seed;
	m_error   = 0;
	lam::common::avx256_init_unroll8x(&m_rvec[0], static_cast<int64_t>(m_nvalues), 0.0L); // m_rvec should be in cache
}
...
MKLWeibullRNG(_In_ const MKLWeibullRNG &x) {
	...
	m_rvec  = lam::common::lam_edmalloca(static_cast<size_t>(x.m_nvalues), align64B);
	m_alpha = x.m_alpha;
	m_a     = x.m_a;
	m_beta = x.m_beta;
...
compute_rand_distribution(_In_ const MKL_INT method) {
	...
	m_error = vdRngWeibull(method, stream, m_nvalues, &m_rvec[0],m_alpha,m_a,m_beta);
	if (VSL_ERROR_OK != m_error) {
		PRINT_ERROR_VALUE("vdRngWeibull -- failed with an error value: ", m_error)
		DUMP_CALLSTACK_ON_ERROR
		return;
	}
...
operator<<(_In_ std::ostream &os,
	...
	for (MKL_INT i = 0; i != x.m_nvalues; ++i) {
		os << std::setprecision(15) << std::showpoint <<
			"Random Weibull distribution: m_rvec = " << x.m_rvec[i] << std::endl;
	}
	return (os);
}