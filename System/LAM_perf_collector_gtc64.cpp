...
#include <iostream>
#include <iomanip>
#include "LAM_perf_collector_gtc64.h"
...
::compute_stats(_Inout_ double &mean,
	...
	for (size_t i = 0Ui64; i != m_delta_values.size(); ++i) { // <-- no auto-vectorization (no countable loop)
		if (zero != m_delta_values.operator[](i)) {
			...
			r8_loc.t   = std::abs(r8_loc.tmp - mean); //potential catastrophic cancelltation if(tmp - mean) both are very close to each other.
...
::correlation_set(_In_ PerfCollectorGTC64 &set1,
	...
	PERF_COLLECTOR_GTC64_DFTI_FAIL_CLEANUP(status,dataset1,dataset2,ddh2)
