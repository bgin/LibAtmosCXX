...
#include <iostream>
#include <iomanip>
#include "LAM_rdtscp_timer.h"
#include "../Math/LAM_constants.h"
#include "../LAM_indices.h"

lam::system
::RDTSCPTimer
::RDTSCPTimer(_In_ const std::size_t nruns,
...
lam::system
::RDTSCPTimer
::RDTSCPTimer(_In_ const RDTSCPTimer &x)
:
m_nruns{ x.m_nruns },
...
lam::system
::RDTSCPTimer
::RDTSCPTimer(_In_  RDTSCPTimer &&x)
:
m_nruns{ x.m_nruns },
m_datum_size{ x.m_datum_size },
m_ptfunc{ x.m_ptfunc },
...
lam::system::RDTSCPTimer &
lam::system::RDTSCPTimer
::operator=(_In_ const RDTSCPTimer &x) {
...
#if !defined (LAM_RDTSCP_TIMER_CHECK_ARGS_CONFORMANCE)
#define LAM_RDTSCP_TIMER_CHECK_ARGS_CONFORMANCE(vres,set1,set2)                      \
do{																                     \
...
::delta_values_eq(_Out_ std::vector<bool> &vres,
				  _In_ const RDTSCPTimer &set1,
				  _In_ const RDTSCPTimer &set2) {
	LAM_RDTSCP_TIMER_CHECK_ARGS_CONFORMANCE(vres, set1, set2)
