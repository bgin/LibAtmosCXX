...
#if (USE_AUTO_VEC) == 1
        double * __restrict t1 = &rhs.m_StormSpeed[0]; // Workaround for lack of pragma prefetch on STL objects.
		double * __restrict t2 = &rhs.m_StormDir[0];   // Workaround for lack of pragma prefetch on STL objects.
#pragma prefetch t1:0:2 
#pragma prefetch t2:0:2
#pragma simd
#pragma unroll(4)
	for (int32_t i = 0; i != m_nsize; ++i) {
		m_StormSpeed[i] = t1[i];
