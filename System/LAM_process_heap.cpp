...
#include "LAM_process_heap.h"
//
//	Implementation
//

lam::system::ProcessHeapInfo::
...
int lam::system::ProcessHeapInfo::get_nstates() const {
	return (this->m_nstates);
}

WORD lam::system::ProcessHeapInfo::get_flags() const {
	return (this->m_flags);
}

HANDLE lam::system::ProcessHeapInfo::get_heapHandle() const {
	return (this->m_heapHandle);
}

HANDLE lam::system::ProcessHeapInfo::get_procHandle() const {
	return (this->m_procHandle);
}

DWORD lam::system::ProcessHeapInfo::get_procID() const {
	return (this->m_procID);
}

bool  lam::system::ProcessHeapInfo::get_heap_lock() const {
	return (this->m_lock_heap);
}

// beware of object lifetime here!!
// Reference maybe dangling when object will be destroyed.
const std::list<lam::system::ProcessHeapInfo::heap>  & 
lam::system::ProcessHeapInfo::get_heapList() const {
	return (this->m_heapList);
}

 std::list<lam::system::ProcessHeapInfo::heap>::const_iterator
lam::system::ProcessHeapInfo::get_heapListIterator() const {
	return (this->m_heapList.cbegin());
}

void lam::system::ProcessHeapInfo::print_heap_state() const{
...
void  lam::system::ProcessHeapInfo::heap_enumerate() {
	DWORD hwerr = 0;
	BOOL bhl = false;
	BOOL bhw = false;
