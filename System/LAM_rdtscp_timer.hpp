...
#if !defined (LAM_RDTSCP_TIMER_DESCRIPT)
#define LAM_RDTSCP_TIMER_DESCRIPT " RDTSCP based timer class."
#endif
...
namespace lam {
	namespace system {
		...
		template<class _Functor>  struct RdtscpTimer {
			...
			std::size_t get_datum_size() const { return (m_nruns * m_datum_size); }
			...
			bool compute_stats(_Out_ double &mean,
				...
				for (size_t i = 0Ui64; i != m_timing_values.size(); ++i) {
					if (zero != m_timing_values.operator[](i)) {
						...
						if (r8_loc.fracp <= DEPS) std::printf("compute_stats: Losing a significant digits: %.16f, result may not be an accurrate\n",r8_loc.fracp);
