...
#define LAM_PERF_COLLECTORS_DESCRIPT " Code section performance collector class based on RDTSCP machine-code instruction."
#endif
...
namespace lam {
	namespace system {
		     ...
		     struct PerfCollectorRDTSCP {
				...
				PerfCollectorRDTSCP(_In_ const PerfCollectorRDTSCP &x) noexcept(false)
					:
				...
				PerfCollectorRDTSCP(_In_ PerfCollectorRDTSCP &&x) noexcept(true)
					:
				...
				PerfCollectorRDTSCP & operator=(_In_ const PerfCollectorRDTSCP &x) {
				...
				PerfCollectorRDTSCP & operator=(_In_ PerfCollectorRDTSCP &&x) {
					if (this == &x) return (*this);
					    m_Iscleared = std::move(x.m_Iscleared);
				     	m_nsamples = std::move(x.m_nsamples);
				...
				bool static correlation_set(_In_   PerfCollectorRDTSCP &set1,
