...
::print_object_state() const {
	...
	for (int32_t i = 0; i != m_np; ++i) {
		for (int32_t j = 0; j != m_nt; ++j) {
			std::cout << std::setprecision(15) << std::showpoint << std::setw(4) <<
				m_prdist[Ix2D(i, m_np, j)] << std::setw(8) << m_ptheta[Ix2D(i,m_np,j)] <<
				std::setw(12) << m_pphi[Ix2D(i, m_np, j)] << std::setw(16) << m_pvf[Ix2D(i,m_np,j)] << "\n";
		}
	}
	...
	for (int32_t i = 0; i != m_np; ++i) {
		for (int32_t j = 0; j != m_maxx; ++j) {
			std::cout << std::setprecision(15) << std::showpoint << std::setw(4) <<
				m_ppx[Ix2D(i, m_np, j)] << std::setw(8) << m_ppy[Ix2D(i, m_np, j)] <<
				std::setw(12) << m_ppz[Ix2D(i, m_np, j)] << "\n";
		}
	}
	std::cout << " m_xsect \n";
	for (int32_t i = 0; i != m_np; ++i) {
	...
	if (curve_type == true) {
	    for (int32_t i = 0; i != m_np; ++i) {
		      ...
		      for (int32_t j = 0; j != m_maxsp; ++j) {
			       ...
			       m_xsect[Ix2D(i, m_np, j)] = sphrad[i] * term;
		      }
	     }
	}
				...
				for (int32_t ix = 0; ix != nx; ++ix) {
					for (int32_t iy = 0; iy != ny; ++iy) {
						for (int32_t iz = 0; iz != nz; ++iz) {
							 tmp1 = 1.0 - 2.0 * bRet[Ix2D(i,m_np,j)];
							 term1 = aRet[Ix2D(i, m_np, j)] * std::pow(kvisc[Ix3D(ix,nx,iy,ny,iz)],tmp1);
							 term2 = (2.0 * vb[i] * 9.81) / A[i];
							 if (rho_b[i] > rho_f[Ix3D(ix, nx, iy, ny, iz)]) 
								 term2a = std::abs(rho_b[i] / rho_f[Ix3D(ix, nx, iy, ny, iz)]);
							 else
								 term2a = std::abs(rho_b[i] / rho_f[Ix3D(ix, nx, iy, ny, iz)] - 1.0);
							 tmp2 = 2.0 * bRet[Ix2D(i,m_np,j)] - 1.0;
							 term3 = std::pow(mD[i],tmp2);
							 m_pvf[Ix2D(i, m_np, j)] = term1 * std::pow(term2 * term2a, bRet[Ix2D(i,m_np,j)]) * term3;
						}
					}
				}
			}
		...
		for (int32_t i = 0; i != m_np; ++i) {
			for (int32_t j = 0; j != m_nt; ++j) {
				...
				for (int32_t ix = 0; ix != nx; ++ix) {
					for (int32_t iy = 0; iy != ny; ++iy) {
						for (int32_t iz = 0; iz != nz; ++iz) {
							tmp1 = 1.0 - 2.0 * bRe[Ix2D(i, m_np, j)];
							term1 = aRe[Ix2D(i, m_np, j)] * std::pow(kvisc[Ix3D(ix, nx, iy, ny, iz)], tmp1);
							term2 = (2.0 * vb[i] * 9.81) / A[i];
							if (rho_b[i] > rho_f[Ix3D(ix, nx, iy, ny, iz)])
								term2a = std::abs(rho_b[i] / rho_f[Ix3D(ix, nx, iy, ny, iz)]);
							else
								term2a = std::abs(rho_b[i] / rho_f[Ix3D(ix, nx, iy, ny, iz)] - 1.0);
							tmp2 = 2.0 * bRe[Ix2D(i, m_np, j)] - 1.0;
							term3 = std::pow(mD[i], tmp2);
							m_pvf[Ix2D(i, m_np, j)] = term1 * std::pow(term2 * term2a, bRe[Ix2D(i, m_np, j)]) * term3;
						}
					}
				}
			}
