...
#if (USE_NT_STORES) == 1
		avx256_memcpy8x_nt(&data.m_rvec[0], &x.data.m_rvec[0], x.data.m_nvalues);
#else
		avx256_memcpy8x(&data.m_rvec[0], &x.data.m_rvec[0], x.data.m_nvalues); // m_nvalues size-extended to long long int will be kept in rdx, so upper 32-bit will be wasted.
#endif
	}
	return (*this);
}	
